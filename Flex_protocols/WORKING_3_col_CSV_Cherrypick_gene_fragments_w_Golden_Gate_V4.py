...
# Here we assume the file is already uploaded and parsed by the Opentrons app 
...
for index, row in enumerate(well_data):
    # Get the source well location from the CSV (well data)
    source_well = row[0]  # Assuming well name in format 'A1', 'B1', etc.
    
    # Access the source well location from the source plate
    source_location = source_plate[source_well]

    # Get the transfer volume from the CSV and convert it to float
    transfer_volume = float(row[2])

    # Get the destination well location from the CSV
    destination_well = row[1]  # Assuming destination well name in format 'A1', 'B1', etc.
    
    # Access the destination well location from the destination plate
    destination_location = destination_plate[destination_well]

    # Perform the transfer using pipette50
    pipette50.transfer(
